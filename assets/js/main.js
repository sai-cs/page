...
(function() {
  ...
  document.getElementById("twitter-btn").addEventListener("click", function (e) {
    e.preventDefault();
    showPopup();
  });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  pureCounters.forEach((counter) => {
    ...
    let current = start;
    const increment = Math.ceil((end - start) / (duration * 60)); // Increment value based on duration
    ...
    const interval = setInterval(updateCounter, 1000 / 60); // Update every frame (60 FPS)
  });
});
